...
public class MultiPlayerGame {
    ...
    private void moveGhosts(){
        ...
        for (Ghost ghost: ghosts){
            ...
            if (CollisionChecker.collisionWithGhost(ghost, yellowPacman)) {
                AnimationBlinker.triggerDyingAnimationForMultiPlayerGame(yellowPacman, 1,gamePanel);
            }
    
            if (CollisionChecker.collisionWithGhost(ghost, redPacman)) {
                AnimationBlinker.triggerDyingAnimationForMultiPlayerGame(redPacman, 2, gamePanel);
            }
            ...
            if( (ghost.getX() == MultiPlayerGameBoard.getTileSize()*6 || ghost.getX() == MultiPlayerGameBoard.getTileSize()*12 ) && ghost.getDirection() != 'L' && ghost.getDirection() != 'R'){
                ...
                MovementManager.updateDirection('L',walls,ghost);
            }
    ...
    public void draw(Graphics g){
        ...
        if (redPacman.isPacmanDying()) {
            ...
            if (AnimationBlinker.getCurrentFrame() < DeathFramesHandler.getTotalFramesCount()) {
                g.drawImage(DeathFramesHandler.getDyingFrameForRed()[AnimationBlinker.getCurrentFrame()], redPacman.getX(), redPacman.getY(), redPacman.getDimensions().getWidth(), redPacman.getDimensions().getHeight(), null);
            }
        } 
